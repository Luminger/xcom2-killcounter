 ...
 */
class XComWaitCondition_Or extends SeqAct_XComWaitCondition;

var() editinline SeqAct_XComWaitCondition Condition1;
var() editinline SeqAct_XComWaitCondition Condition2;

/**
 * @return True if either or both of the start conditions are true. None treated as no condition.
 */
...
event string GetConditionDesc()
{
	...
	{
		if (Condition2 != none)
			Condition = "(" $ Condition1.GetConditionDesc() $ " OR " $ Condition2.GetConditionDesc() $ ")";
		else
...
DefaultProperties
{
	ObjName="Wait for <Condition1> OR <Condition2>"
}
