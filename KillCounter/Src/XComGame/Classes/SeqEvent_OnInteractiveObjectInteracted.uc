class SeqEvent_OnInteractiveObjectInteracted extends SeqEvent_X2GameState;
...
static function FireEvent(XComGameState_InteractiveObject InteractiveObjectState, XComGameState_Unit Unit)
{
	...
	local SeqEvent_OnInteractiveObjectInteracted Event;
	...
	GameSeq.FindSeqObjectsByClass(class'SeqEvent_OnInteractiveObjectInteracted', true, Events);
	for (Index = 0; Index < Events.length; Index++)
	{
		Event = SeqEvent_OnInteractiveObjectInteracted(Events[Index]);
		if( Event != None )
		{
...
defaultproperties
{
	ObjName="On Interactive Object Interacted";
