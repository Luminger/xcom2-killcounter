...
{	
	...
	if( ActiveUnitRef.ObjectID > 0 && UpgradeDropdown.SelectedItem > -1 )
	{
		...
		if (bClearUpgradeMode || (UpgradeTemplate != none && UpgradeTemplate.CanApplyUpgradeToWeapon(WeaponStateObject)))
		{
			...
			WeaponStateObject = XComGameState_Item(ChangeState.CreateStateObject(class'XComGameState_Item', WeaponStateObject.ObjectID));
			...
			ChangeState.AddStateObject(WeaponStateObject);
...
simulated function PopulateUpgradeDropdown(UIDropdown kDropdown)
{
	...
	for( Index = 0; Index < WeaponUpgrades.Length; ++Index )
	{
		...
		if( WeaponStateObject != none && WeaponStateObject.ObjectID > 0 && !WeaponUpgrades[Index].CanApplyUpgradeToWeapon(WeaponStateObject) )
		{
			continue;
		}
